...
export default function Modal({ children, full = false, onCloseModal, wrapClassName = "", isExit = false, exitStyle = "", backDropEvent = () => {}, className = "", backDropClassName = "", maxWidth = "" }: Props) {
  return (
    <div className={`${wrapClassName} fixed inset-0 z-40 `}>
      ...
      <div className={` fixed z-50  ${full ? "bottom-0" : "top-2/4 -translate-y-2/4"} w-full flex justify-center z-50 `}>
        <div className={`${maxWidth} w-full ${full ? "px-0" : "px-4"}`}>
          <div className={`${className} w-full  pb-[2px]  ${full ? "rounded-t-lg" : "rounded-lg"}`}>
            {isExit && (
              <div className={`flex w-full items-center justify-start ${exitStyle} `}>
                <i className="fa-solid fa-xmark text-2xl" onClick={onCloseModal}></i>
              </div>
            )}
            {children}
          </div>
